...
export default function StayPreviewSection() {
  return (
    ...
    >
      <div
        aria-hidden
        className="pointer-events-none absolute inset-0 bg-[linear-gradient(to_bottom,rgba(21,35,56,0.05)_0%,rgba(21,35,56,0.02)_32%,rgba(255,255,255,0)_100%)]"
      />
      ...
      <div className="relative mx-auto grid w-full max-w-6xl grid-cols-1 gap-7 px-6 py-14 sm:px-10 sm:py-16 lg:grid-cols-[minmax(0,1.08fr)_minmax(0,0.92fr)] lg:items-center lg:gap-10 lg:px-16 lg:py-20">
        <Link
          ...
          className="group relative block min-h-[19rem] overflow-hidden rounded-2xl border border-[color:var(--border-soft)] bg-[#ddd4c8]"
        >
          <Image
        ...
        <div>
          ...
          <h2 className="mt-3 max-w-md text-2xl font-semibold leading-tight tracking-tight text-[var(--home-heading)] sm:text-3xl">
            쉬어가기 좋은 달숨의 숙박 공간
          </h2>
          ...
          <div className="mt-5 space-y-2.5">
            {stayPoints.map((point) => (
              <p
                key={point}
                className="border-l pl-3 text-sm text-[var(--text-sub)]"
                style={{ borderColor: "var(--home-muted-line)" }}
              >
                {point}
              </p>
            ))}
